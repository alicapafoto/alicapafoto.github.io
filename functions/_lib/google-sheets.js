...
export async function hasOrderSession(env, sessionId) {
  ...
  }, 12_000, "Google Sheets order lookup");
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(payload?.error?.message || "Could not check the Google order ledger");
...
export async function appendOrderRow(env, values) {
  ...
  }, 15_000, "Google Sheets order append");
  const payload = await response.json().catch(() => ({}));
