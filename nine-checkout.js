(() => {
  ...
  async function loadCatalogue() {
    try {
      ...
      openButton.textContent = "Europe Â· temporarily unavailable";
    }
  }
  ...
  async function getQuote() {
    ...
    setBusy(true, "Finding delivery optionsâ€¦");
    try {
      ...
      quote = payload;
      quoteBox.innerHTML = `<div class="store-quote__row"><span>NiNE Â· European Softcover</span><strong>${euro(payload.priceCents)}</strong></div>
  ...
  async function startCheckout() {
    ...
    showMessage();
    setBusy(true, "Preparing secure checkoutâ€¦");
    try {
