...
## 9. Collector-edition shipping activation

Collector checkout deliberately stays disabled until verified shipping charges are supplied. Add euro-cent values for both enabled regions:

- `KAISAR_PRESENCE_SHIPPING_EU_CENTS`
- `KAISAR_PRESENCE_SHIPPING_US_CENTS`
- `KAISAR_IMMERSION_SHIPPING_EU_CENTS`
- `KAISAR_IMMERSION_SHIPPING_US_CENTS`
- `MERANO_REVERIE_SHIPPING_EU_CENTS`
- `MERANO_REVERIE_SHIPPING_US_CENTS`

Do not add `RAABTA_VEIL` values until the physical proof is approved and the catalogue availability is changed from `proof-hold` to `available`.

These values are the full customer-facing shipping-and-handling charge, in euro cents. Verify them with a real manual Creativehub quote for each size and region. The code refuses checkout when a value is missing rather than inventing a rate.
